...
module Tables {
    ...
    export class Basic {
        ...
        //    be enhanced.
        //
        public constructor(selector: string | JQuery) {
            $(() => {
                this.$table = $(selector as any);
                this.Configure();
            });
        }
        ...
        public Configure(): void {
            ...
            this.$table.each((idx, table) => {
                ...
                if (UsesSelection) {
                    ...
                    this.ConfigureSelection(table);
                    this.ConfigureSelectionPersistance(table);
                    if (UsesSecondaryRow) {
        ...
        private ConfigureColumnResizing(table) {
        ...
        //
        private ConfigureSelection(table: HTMLElement) {
            //
            // Bind row selection checkboxes and clicking on row
            //
            $(table).on('click', 'thead input[type=checkbox]', (e) => this.OnClickSelectAll(e));
            $(table).on('click', 'tbody input[type=checkbox]', (e) => this.OnClickCheckbox(e));
            $(table).on('click', 'tbody input[type=radio]', (e) => this.OnClickCheckbox(e));
            $('> tbody > tr', table).on('click', (e) => this.OnClickRow(e));
        }

        //
        // Bind the delete button (might be in a modal dialog if confirmation was required)
        //
        private ConfigureDeleteButton() {
            if ($('#cmdDelete').data('bs-toggle') !== 'modal') {
                ...
                $($('#cmdDelete').data('bs-target') + ' .btn-danger').click((e) => {
                    (new (window as any).bootstrap.Modal($('#ConfirmDelete')[0])).hide();
        ...
        //
        private ConfigureSelectionPersistance(table: HTMLElement) {
            //
            // These are links inside the table
            //
            $(table).on('click', 'thead a', (e) => this.OnClickNavigationLink(e, table));
            $(table).on('click', 'tbody a', (e) => this.OnClickNavigationLink(e, table));

            //
            // These are standard links outside the table
            //
            $('#cmdNew').on('click', (e) => this.OnClickNavigationLink(e, table));
            $('#PageSizeSelection a').on('click', (e) => this.OnClickNavigationLink(e, table));
        }

        //
        // When a checkbox is clicked raise the rowselected event
        //
        private OnClickCheckbox(e) {
            this.SetUi();
        ...
        //    or a link
        //
        private OnClickRow(e: JQuery.ClickEvent) {
            var target = $(e.target);
        ...
        // 
        private OnClickNavigationLink(e: JQuery.ClickEvent, table: HTMLElement) {
            //
        ...
        //
        private GetSelectedItemsListParameter(table: HTMLElement) {
            //
        ...
        //
        private OnClickSelectAll(e: JQuery.ClickEvent) {

            var checked = e.target.checked;

            $(e.target).closest('TABLE').find('TBODY input').each((idx, elm) => {
                (elm as any).checked = checked;
            });
